...
const PasswordInput: React.FC<PasswordInputProps> = ({ minLength = 8, maxLength = 12, requiresSpecialChar = true, requiresNumber = true, placeholder }) => {
    ...
    useEffect(() => {
        if (password) {
            if (password.length < minLength || password.length > maxLength || (requiresSpecialChar && !/[!@#$%^&*]/.test(password)) || (requiresNumber && !/\d/.test(password))) {
    ...
    return (
        <Form>
            <Input type="password" value={password} onChange={handlePasswordChange} placeholder="8-12자, 영문, 특수문자, 숫자 포함" autoComplete="new-password" />
            <Text>{specialCharMessage}</Text>

            <Input type="password" value={confirmPassword} onChange={handleConfirmPasswordChange} placeholder="비밀번호 확인" autoComplete="new-password" />
